...
/**
 * Springboot Jpa 自动创建表 和字段乱序问题 在本项目中创建一个和源码类一样的包结构和一样名字的类
 * 这个类把此类中所有的TreeMap替换成LinkedHashMap 就变成有序的了
 *
 * @author chenyacheng
 */
class PropertyContainer {
	...
	/**
	 * Holds the AccessType indicated for use at the class/container-level for cases
	 * where persistent attribute did not specify.
	 */
	...
	PropertyContainer(XClass clazz, XClass entityAtStake, AccessType defaultClassLevelAccessType) {
		...
		if (defaultClassLevelAccessType == AccessType.DEFAULT) {
			// this is effectively what the old code did when AccessType.DEFAULT was passed
			// in
		...
		this.classLevelAccessType = localClassLevelAccessType != AccessType.DEFAULT ? localClassLevelAccessType
	...
	private void collectPersistentAttributesUsingLocalAccessType(
		...
		while (propertyIterator.hasNext()) {
			...
			// HHH-10242 detect registration of the same property getter twice - eg boolean
			// isId() + UUID getId()
	...
	private AccessType determineLocalClassDefinedAccessStrategy() {
		...
		org.hibernate.annotations.AttributeAccessor accessType = xClass
				.getAnnotation(org.hibernate.annotations.AttributeAccessor.class);
		if (accessType != null) {
